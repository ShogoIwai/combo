...
4. **Adjudicate F/I/V（LLM）.** `work/ledger.json` の**全行**を埋める。行を消したり間引いたりしない。
   ...
   - `value`:
     ...
     - `origin`: その基準が**自分のどこから来ているか**。立場ファイルの `## 検査用アンカー` の語を含めて書く（G8）。
...
7. **Write the opinion.** `reference/opinion_template.md` の章立てで 1 本の md を書く。
   ...
   - §2 は解釈を**1 項目 = 1 件**（見出しに担当 CID を 1 個）で全件。`- 事実:`（台帳 `basis` と
     完全一致）/ `- 解釈:` / `- 他の見方:` を置き、台帳の `interpretation` / `alternative` を
     **その項目の中に**逐語で入れる（別項目への寄せ集めは G11 で落ちる）。
   - §3 も**1 項目 = 1 件**で価値基準を全件。`- 軸:` / `- 由来:` / `- トレードオフ:`。
   - §4 は `- 意見:` / `- 根拠:`（CID 2 件以上）/ `- 前提の価値基準:`（§3 の CID）/
     `- 私はこうする:` / `- この意見が変わる条件:`。立場の語が 2 つ以上出ること。
   - 付録A（§5）に UNVERIFIED を全件、**1 行 = CID 1 個**（探した範囲には `searched` の
     queries と domains の**両方**の語を入れる）。付録B（§6）に反論と応答を 2 件以上。
...
9. **Verify.**
   ...
   ```
   `--inputs/--stance` を渡すと manifest のパス申告を信用せずその入力で再現照合する。
   **省略すると manifest の申告どおりの入力で照合する**ので、`--inputs` を渡すこと（強く推奨）。
...
| G1 | 台帳の型（複合フィールドの型を先に固定。壊れていればここで打ち切る）と網羅（1 claim = 1 行、ID 重複なし、行数 = manifest） | 面倒な claim を台帳から消して 0 件 PASS／型を壊して checker を traceback で殺す |
| G2 | 未裁定 0（kind/kind_rationale＋種別ごとの必須欄と最低字数） | 空欄のまま「整理済み」と称する |
| G3 | `restated` が元の記述の写経でない | 言い直さず原文コピーで裏取り済みを装う |
| G4 | 公開出典の実体（URL 書式・実取得 status=200・**証跡ファイル名が URL の sha256 と一致**・`text_sha256` 一致・`final_url`/`fetched`/`sha256` の実在・ページ内に実在する引用・**実在する日付**） | URL の作文、実在ドメイン＋架空パス、証跡テキストの手編集、取得していない URL、**別 URL の証跡の流用**、`2026-99-99` |
...
| G8 | 価値基準の由来が立場ファイルの**検査用アンカー**に紐づき、`tradeoff` が由来の言い換えでない、`value` が 1 件以上 | 誰にでも当てはまる一般論を「自分の価値基準」と称する、捨てるものを書かずに良いことだけ並べる |
| G9 | 章立て（順序・正確な表題）と 付録C〜E（§7〜§9）の**章ごと**逐語一致 | 生成物を手で書き換えて件数を合わせる |
| G10 | §1 は `###` テーマ配下・裏取り済み fact が**全件**本文に出る・**CID を置いた記述にはその CID 全部の** `restated` が逐語・1 記述に CID は 3 個まで・事実以外の CID なし・私見表現ゼロ | 取りこぼし、解釈や価値観を事実編に紛れ込ませる、台帳をベタ貼りして ID だけ並べる |
| G11 | §2 が解釈と**項目 1 対 1**（見出しに担当 CID 1 個・集合完全一致）、各項目に 事実(台帳 `basis` と一致)/解釈(60字)/他の見方(30字)、台帳の文言が**その項目内に**逐語 | 都合の悪い解釈を落とす、対立解釈を書かずに断定する、**1 項目に全部を寄せて他は CID だけの空見出しにする** |
| G12 | §3 が価値基準と**項目 1 対 1**（見出しに担当 CID 1 個・集合完全一致）、各項目に 軸/由来/トレードオフ、台帳の文言が**その項目内に**逐語 | 価値基準を本文から消して「客観的な分析」に見せる、寄せ集め |
| G13 | §4 に 意見(40字)/根拠(CID 2 件以上)/前提の価値基準(§3 の CID)/私はこうする(30字)/この意見が変わる条件(30字)、立場の語 2 つ以上 | 反証条件のない断定、根拠を示さない結論、自分の行動を伴わない評論 |
| G14 | 付録A に UNVERIFIED **のみ**全件、**1 行 = CID ちょうど 1 個**、探した範囲が台帳 `searched` の queries **と** domains の両方に対応 | 未確認を黙って本文へ混ぜる、探索範囲を作文する、**1 行に複数件を詰めて「1 行 1 件」判定を空振りさせる** |
| G15 | 付録B に反論 2 件以上、`- 反論:` `- 応答:` 各 40 字以上、複製なし | 反論欄に弱い藁人形を 1 つ置いて済ませる |
| G16 | placeholder（〈…〉/TODO、HTML タグ・autolink でない `<…>`）ゼロ / §0〜§6 の件数は台帳の集計 ∪ 根拠テキストに実在する件数だけ | TODO 残し、手集計の件数 |
| G17 | 散文主体（§0 地の文 120 字・§1 全体 200 字＋テーマごと 40 字・§2 各項目 150 字・§4 150 字・§0〜§4 の表比率 40% 以下） | 箇条書きと表の羅列で済ませる、台帳を本文の代わりにする |
| G18 | **逆分類の説明責任**。fact-like な原文（数値・日付・単位・発表/採用等の動詞・「〜によると」・URL）を interpretation/value にするなら `kind_rationale` 40 字以上。規範語（べき・望ましい・優先 等）を含む記述を fact にする場合も同じ | **裏取りが面倒な fact を解釈・価値観へ逃がす／価値判断を事実に偽装する**（この skill で最も起きやすい 2 つの迂回） |
...
- 出典が主張を本当に裏づけているか（G4 は URL・証跡・引用の**実在**しか見ない）
- `fact` / `interpretation` / `value` の裁定が妥当か。G18 が課すのは**説明責任だけ**で、
  意味的な妥当性は判定しない（表層シグナルのある逆分類に厚い理由を要求することで、
  「価値判断を事実に偽装する」「裏取りが面倒な事実を解釈・価値観へ逃がす」のコストを
  上げているにすぎない。シグナルの無い言い換えで書かれれば素通りする）

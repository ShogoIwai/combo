...
check_note.py の G7 で不合格になるので、台帳の出典を差し替えるか、
その行を UNVERIFIED / PRIVATE_PRIMARY へ裁定し直すこと。
**証跡を手で書き足して通すのは goal 未達の隠蔽。** (G7 は text_sha256 も照合する)
...
UA = "Mozilla/5.0 (compatible; combo-event-truth-note/1.0)"
...
def main() -> None:
    ...
    urls: list[str] = []
    # 台帳（事実のファクトチェック）と digs.json（深掘り調査）の両方から集める
    dp = work / "digs.json"
    digs = json.loads(dp.read_text(encoding="utf-8")) if dp.is_file() else []
    for r in list(rows) + list(digs):
        for s in r.get("sources") or []:
            if isinstance(s, dict) and s.get("url") and s["url"] not in urls:
                urls.append(s["url"])
    if not urls:
        die("台帳にも digs.json にも出典 URL が 1 つも無い — 先にファクトチェックと深掘りを済ませること")

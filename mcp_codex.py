#!/usr/bin/env python3
"""MCP server: fork a task from Claude Code into a one-shot Codex run,
...
whatever Codex itself is configured to use (`~/.codex/config.toml`) unless
CODEX_MODEL pins one. This is the *cloud* Codex path; it does not read OPENAI_* from source_local.
...
# Model pin. Empty -> whatever Codex is configured to use (~/.codex/config.toml).
...
def fork_to_codex(task: str, repo: str = "", sandbox: str = "workspace-write") -> str:
    """Fork a self-contained coding task to Codex, sandboxed to one repo.
...
def ask_codex(question: str, repo: str = "") -> str:
    """Ask Codex a read-only question about a repository.
...
def web_rag(query: str, repo: str = "") -> str:
    """Answer a question using Codex with live web search (grounded RAG).
